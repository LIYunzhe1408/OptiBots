...
from random_env_generation.random_env_generation import plot_random_cuboids
...
class Reachability:
    ...
    def reachability_random_sample_actors(self, num_samples, num_actors = 4, batch_size = 1000, debug = False):
        ...
        def pin_robot_factory():
            ...
            return ModuleAssembly.from_serial_modules(db, self.robot_modules).to_pin_robot()
        
        # Create actors
        actors = []
        for _ in range(num_actors):
            actors.append( ReachabilityWorker.remote(robot_create = pin_robot_factory, task_create = self.task, mass = self.mass))
...
if __name__ == "__main__":
    ...
    # Create database with our URDF modules
    eef = create_eef()
